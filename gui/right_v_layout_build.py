...
def right_v_layout_build(self):
    ...
    self.status_browser_bar = status_browser_bar_build(self)
...
def browser_window_build(self):
    ...
    browser_stacked_window.browser.url = "https://www.example90.com"
    self.general.browsers.append(browser_stacked_window.browser)
    self.general.current_browser = browser_stacked_window.browser
...
def status_browser_bar_build(self):    
    browser= self.browser_stacked_window.browser
    ...
    go_btn = style_btn("Go")
    go_btn.clicked.connect(lambda: navigate(self, "https://www.example2.com"))
    back_btn=style_btn("back")
    back_btn.clicked.connect(self.general.current_browser.back)
    forward_btn =style_btn("forward")
    forward_btn.clicked.connect(self.general.current_browser.forward)
    hello_btn = style_btn("")
    hello_btn.clicked.connect(self.general.current_browser.forward)
